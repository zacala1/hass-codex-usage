...
베타 버전은 기본적으로 일반 업데이트에 표시되지 않습니다. 베타 테스트에
참여하려면 해당 저장소의 HACS 프리릴리스 스위치 엔티티를 활성화한 뒤
**Update information**을 실행하세요. 정식 버전만 사용하려면 이 스위치를
꺼진 상태로 유지하세요.
...
Beta releases use tags such as `v0.3.2b1` and are published with GitHub's actual
pre-release flag. The HACS pre-release switch for each repository is disabled by
default. Enable that switch and refresh the repository information to receive
beta updates; leave it disabled to remain on the latest stable release.
...
The listed sensors are enabled when the account response contains their
...
Sensor availability is evaluated after the first successful usage response.
The integration preserves entities disabled manually by the user. If OpenAI
later enables a previously absent feature for the account, reload the integration
or restart Home Assistant to evaluate the supported sensors again.
...
For a beta release, give the manifest and `const.py` a version such as
`0.3.2b2`, then use the matching tag `v0.3.2b2` (generally
`v<base version>b<number>`). The workflow marks that release as a prerelease;
the final `v0.3.2` release remains a separate stable version.

After publication, verify that a stable release is marked Latest or that a beta
release is not, as appropriate. Also verify that the published ZIP contains only
the reviewed root-level integration files and that its SHA-256 digest matches a
fresh local deterministic build.
